...
namespace Postieri.Controllers
{
    ...
    public class WebSocketsController : ControllerBase
    {
        ...
        public WebSocketsController(ILogger<WebSocketsController> logger,
             WebSocketServerConnectionManager manager, DataContext context)
        {
            _logger = logger;
            _manager = manager;
            _context = context;
        ...
        public async Task Get()
        {
            if (HttpContext.WebSockets.IsWebSocketRequest)
            {
                ...
                await Receive(webSocket, async (result, buffer) =>
                {
                    ...
                    {
                        _logger.Log(LogLevel.Information, "Receive->Text");
                        _logger.Log(LogLevel.Information, "Message: {Encoding.UTF8.GetString(buffer, 0, result.Count)}");
                    ...
                    else if (result.MessageType == WebSocketMessageType.Close)
                    {
                        ...
                        _logger.Log(LogLevel.Information, "Receive->Close");

                        _manager.GetAllSockets().TryRemove(id, out WebSocket sock);
                        _logger.Log(LogLevel.Information, "Managed Connections: " + _manager.GetAllSockets().Count.ToString());
        ...
        private async Task RouteJSONMessageAsync(string message)
        {
            ...
            {
                _logger.Log(LogLevel.Information, "Targeted");
            ...
            {
                _logger.Log(LogLevel.Information, "Invalid Recipient");
            }
